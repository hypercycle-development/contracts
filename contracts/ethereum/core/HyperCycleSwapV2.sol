...
/**
    ...
    @notice This contract is an updated version of the c_HyPC/SwapV1 contract for HyperCycle. While the
            ...
            system, V2's assignments focus on assigning numbers to the c_HyPC tokens instead of srings, 
            ...
            we allowed for merging (ie: unspliting) of tokens, then we wouldn't be able to guarantee
            ...
            as previously mentioned. Secondly, we also have limits on how any tokens can be split
...
///@dev Errof for when skipLevels during the split call is 0.
...
error TokenLevelAlreadyHasEnoughTokens();

///@dev Error for when trying to call a function more than once in a block.
error FunctionAlreadyCalledInThisBlock();

contract HyperCycleSwapV2 is ERC721Enumerable, ERC721Holder, Ownable, IHYPCSwapV2, ReentrancyGuard {
    ...
    /** @dev The soft limit of how many tokens to be create at a level.
    ...
    */
    ...
    /// @dev The last block number that assignNumber/assignString function was called in for a tokenId
    ///       this also functions as a way to get how long an assignment has existed for.
    mapping(uint256 => uint256) public _lastBlockCalled;
    ...
    /// @param index: The index that was used for the split in the levels[level] array.
    ...
    event SplitHeldTokens(uint256 tokenNumber, uint256 level, uint256 index, uint256 skipLevels);
    ...
    /// @param index: The index of the tokenNumber in the levels array.
    /// @param hypcAmount: The amount of HyPC locked for this swap.
    event Swap(uint256 tokenNumber, uint256 level, uint256 index, uint256 hypcAmount);
    ...
    /// @param level: The token height for this check (MIN_LEVEL to 19).
    ...
    modifier isCalledInThisBlock(uint256 tokenId) {
        if (_lastBlockCalled[tokenId] == block.number) revert FunctionAlreadyCalledInThisBlock();
        _lastBlockCalled[tokenId] = block.number;
        _;
    }
    ...
    */
    ...
    ///         of this contract transfered to a null address.
    ...
    function addRootTokens(uint256 tokens) external onlyOwner {
        ...
        for (uint256 i; i < tokens; i++) {
            levels[MAX_LEVEL].push(currentRootNumber + i);
            nftTokens[currentRootNumber + i] = TokenData('', 0, MAX_LEVEL, false);

            emit AddRootToken({
                rootNumber: currentRootNumber + i
            });
        }
    ...
    /// @param  index: The index of the token to split inside the levels[level] array.
    /// @param  skipLevels: The number of levels to split down.
    function splitHeldToken(uint256 level, uint256 index, uint256 skipLevels) external validHeldToken(level, index) {
        if (skipLevels <= 0) revert SkipLevelsMustBePositive();
        if (level - skipLevels < MIN_LEVEL) revert TokenLevelTooLowToSplit();

        uint256 tokensToCreate = 2 ** skipLevels;

        if (levels[level - skipLevels].length + tokensToCreate > LEVEL_LIMIT) revert TokenLevelAlreadyHasEnoughTokens();

        uint256[] storage _targetLevel = levels[level];
        //@dev Remove the token at [index] by swapping it with the last element
        uint256 tokenNumber = _targetLevel[index];
        _targetLevel[index] = _targetLevel[_targetLevel.length - 1];
        _targetLevel.pop();
        ...
        for (uint256 i; i < tokensToCreate; i++) {
            levels[level - skipLevels].push(tokenNumber * tokensToCreate + i);
            nftTokens[tokenNumber * tokensToCreate + i] = TokenData('', 0, level - skipLevels, false);
        }
    ...
    /// @param index: The index of the token in the levels[level] array.
    function swapV2(uint256 level, uint256 index) external validHeldToken(level, index) nonReentrant {
        _swapV2(level, index);
    }
    ...
    ///         be minted to the sender, otherwise it will be transfered to them. Ths call will request
    ///         2^level amount of HyPC to get the corresponding c_HyPC.
    /// @param  level: The level of the token to swap for.
    /// @param  index: The index of the token to swap for inside the levels[level] array.
    function _swapV2(uint256 level, uint256 index) internal {
        ...
        if (_targetToken.minted && ownerOf(tokenNumber) == address(this)) {
            _safeTransfer(address(this), _msgSender(), tokenNumber, "");
        } else {
            _targetToken.minted = true;
            _safeMint(_msgSender(), tokenNumber);
        }
        _hypcToken.safeTransferFrom(_msgSender(), address(this), amount * SIX_DECIMALS);
    ...
    function redeem(uint256 tokenNumber) external nonReentrant isOwnerOf(tokenNumber) {
        ...
        _hypcToken.safeTransfer(_msgSender(), amount * SIX_DECIMALS);
    ...
    function assignNumber(uint256 tokenNumber, uint256 targetNumber) external isOwnerOf(tokenNumber) isCalledInThisBlock(tokenNumber) {
        _unassign(tokenNumber);
    ...
    /// @notice Wraper around the _assignString function.
    /// @param  tokenNumber: The c_HyPC to be assigned the targetNumber.
    ...
    /// @param  tokenNumber: The c_HyPC to be assigned the targetNumber.
    /// @param  data: The target string to be assigned.
    function _assignString(uint256 tokenNumber, string memory data) internal isCalledInThisBlock(tokenNumber) {
        _unassign(tokenNumber);
    ...
    function _unassign(uint256 tokenNumber) internal {
        TokenData storage _targetToken = nftTokens[tokenNumber];
        uint256 level = _targetToken.level;

        //if previously assigned, decrease the assignedNumber value
        if (_targetToken.assignedNumber > 0) {
            //unassigned the assigned number
            uint256 assignedNumber = _targetToken.assignedNumber;
            assignedNumbers[assignedNumber] -= 2 ** (level);
        }
        //if previously assigned, decrease the assignedNumber value
        if (bytes(_targetToken.assignedString).length > 0) {
            //unassigned the assigned string
            string memory assignedString = _targetToken.assignedString;
            assignedStrings[assignedString] -= 2 ** (level);
        }
    ...
    /// @notice No longer needed for this version of the CHYPC/Swap contract, but included
    ///         in case calling contract implemented it previously.
    /// @param  tokenId: the id of the token added to the contract. No longer used.
    function addNFT(uint256 tokenId) external {}

    //Getters
    /// @notice Wrapper around getAssignmentString. Mainly for backwards compatibility.
    ...
    function getLastAssigned(uint256 tokenId) external view returns (uint256) {
        return _lastBlockCalled[tokenId];
    }
